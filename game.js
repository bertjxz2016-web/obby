...
function applyPhysics(runner, dt) {
  ...
  const accel = runner.isPlayer ? 25 : aiTune.accelBase + runner.aiMood * aiTune.accelMood;
  const powerSpeed = activePower === "comet" ? 1.18 : 1;
  const aiMaxSpeed = aiTune.speedBase + runner.aiMood * aiTune.speedMood;
  const maxSpeed = (runner.isPlayer ? (carpet ? 22 : 9.2) : aiMaxSpeed) * speedBoost * powerSpeed;
