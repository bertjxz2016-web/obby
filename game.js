...
function courseFormation(index, progress = 0) {
  const blockSize = 16;
  const step = (index - 1) % blockSize;
  const block = Math.floor((index - 1) / blockSize) % 8;
  const t = step / (blockSize - 1);
  const angle = t * Math.PI * 2;
  const harder = progress > 0.6;
  ...
  let stairRun = false;
  let ringRun = false;
  let spiralStair = false;
  let circlePlatform = false;
  let bigSpinnerPlan = false;
  ...
  if (block === 0) {
    ...
    ringRun = true;
    x = Math.cos(angle - Math.PI / 2) * 7.4;
    yStep = 0.25;
    gap = 0.82;
    w = step % 4 === 0 ? 6.3 : 4.9;
    d = w;
    circlePlatform = step === 0 || step === 8 || step === 15;
  } else if (block === 2) {
    spiralStair = true;
    stairRun = true;
    x = Math.cos(angle * 1.1) * (4.2 + t * 3.8);
    yStep = 0.54;
    gap = 0.76;
    w = 5.25;
    d = 5.25;
    circlePlatform = step === 15;
  } else if (block === 3) {
    const court = [0, 2.8, 5.6, 2.8, 0, -2.8, -5.6, -2.8, 0, 3.2, 6.1, 3.2, 0, -3.2, -6.1, -3.2];
    x = court[step];
    yStep = 0.24;
    gap = step % 4 === 0 ? 1.1 : 1.55;
    bigSpinnerPlan = step % 4 === 0;
    circlePlatform = bigSpinnerPlan;
    w = bigSpinnerPlan ? 9.6 : 5.6;
    d = bigSpinnerPlan ? 9.6 : 5.6;
  } else if (block === 4) {
    stairRun = step < 6 || step > 9;
    x = step < 8 ? -4.8 + step * 0.68 : 4.8 - (step - 8) * 0.68;
    yStep = stairRun ? 0.4 : 0.28;
    gap = 1;
    w = 5.75;
    d = 5.55;
  } else if (block === 5) {
    const diamond = [0, 2.4, 4.8, 7, 4.8, 2.4, 0, -2.4, -4.8, -7, -4.8, -2.4, 0, 2.4, 4.8, 2.4];
    ringRun = true;
    x = diamond[step];
    yStep = 0.3;
    gap = 0.9;
    w = step % 4 === 3 ? 5.8 : 4.9;
    d = w;
    circlePlatform = step % 4 === 3;
  } else if (block === 6) {
    stairRun = true;
    spiralStair = true;
    x = Math.sin(t * Math.PI * 1.55) * 6.3;
    yStep = 0.5;
    gap = 0.88;
    w = 5.35;
    d = 5.35;
    circlePlatform = step === 0 || step === 15;
  } else {
    x = Math.sin(angle * 2) * 5.5;
    yStep = 0.32;
    gap = 1.05;
    bigSpinnerPlan = step === 0 || step === 5 || step === 10 || step === 15;
    ringRun = step > 2 && step < 13;
    w = bigSpinnerPlan ? 8.8 : 4.85;
    d = bigSpinnerPlan ? 8.8 : 5;
    circlePlatform = bigSpinnerPlan;
  }

  const shrink = finalStretch ? 0.55 : harder ? 0.28 : 0;
  if (!circlePlatform && !bigSpinnerPlan) {
    w = Math.max(finalStretch ? 3.8 : 4.35, w - shrink);
    d = Math.max(finalStretch ? 4.05 : 4.45, d - shrink * 0.7);
  }
  gap = Math.min(2.85, gap + progress * 0.58 + (finalStretch && !stairRun ? 0.22 : 0));
  yStep += finalStretch && step % 3 === 0 ? 0.08 : 0;

  return { x, yStep, gap, w, d, stairRun, ringRun, spiralStair, circlePlatform, bigSpinnerPlan };
}
...
function buildCourse() {
  ...
  for (let i = 1; i < finalStep; i += 1) {
    ...
    const finalStretch = progress > 0.78;
    const formation = courseFormation(i, progress);
    const ringRun = formation.ringRun;
    const spiralStair = formation.spiralStair;
    const circlePlatform = formation.circlePlatform;
    const stairRun = formation.stairRun;
    ...
    const bigSpinner = allowObstacles && (formation.bigSpinnerPlan || (!ringRun && !spiralStair && (i % 34 === 0 || (hardEnd && i % 23 === 0) || (finalStretch && i % 17 === 0))));
    const hazard = allowObstacles && (i % 7 === 0 || (finalStretch && i % 5 === 0));
    const w = bigSpinner ? Math.max(formation.w, 8.8) : circlePlatform ? Math.max(formation.w, 6.3) : formation.w;
    const d = bigSpinner ? Math.max(formation.d, 8.8) : circlePlatform ? Math.max(formation.d, 6.3) : formation.d;
    const gap = bigSpinner ? Math.max(formation.gap, 1.65) : formation.gap;
    const x = formation.x;
...
function applyPhysics(runner, dt) {
  ...
  const friction = carpet ? 0.93 : disasterType === "slippery" ? 0.985 : runner.onGround ? 0.83 : 0.97;
  ...
  const gravity = carpet ? 2.2 : lowGravity ? 10 : runner.isPlayer && activePower === "feather" && runner.vy < 0 ? 8 : runner.isPlayer ? 21 : 24;
...
function updateRandomDisaster(dt) {
  ...
  if (state.disaster.type === "wind") {
    ...
    for (const runner of [player, ...state.bots]) runner.vx += gust * dt;
    ...
    const shake = Math.cos(performance.now() * 0.015) * 3.2;
    for (const runner of [player, ...state.bots]) {
      runner.vx += rumble * dt;
      runner.vz += shake * dt;
    }
  }
...
function pickDisasterRunner() {
  const runners = [player, ...state.bots].filter((runner) => !runner.finishTime);
  return runners[Math.floor(Math.random() * runners.length)] || player;
}

function spawnMeteor() {
  const runner = pickDisasterRunner();
  const target = state.platforms[Math.min(runner.stage + 2, state.platforms.length - 1)] || state.platforms[runner.stage] || state.platforms[0];
...
function spawnLightning() {
  const runner = pickDisasterRunner();
  const target = state.platforms[Math.min(runner.stage + 1 + Math.floor(Math.random() * 4), state.platforms.length - 1)] || state.platforms[runner.stage] || state.platforms[0];
...
function updateHazardShots(dt) {
  const runners = state.mode.id === "boss" || state.mode.id === "disaster" ? [player, ...state.bots] : [player];
  for (const shot of state.bossShots) {
    shot.life -= dt;
    shot.x += shot.vx * dt;
    shot.y += shot.vy * dt;
...
function ensureEndlessPlatforms() {
  ...
  while (previous.z - player.z < 180) {
    ...
    const progress = Math.min(1, state.endlessCount / 180);
    const formation = courseFormation(state.endlessCount, progress);
    const ringRun = formation.ringRun;
    const spiralStair = formation.spiralStair;
    const circlePlatform = formation.circlePlatform;
    const w = circlePlatform ? Math.max(formation.w, 6.3) : formation.w;
    const d = circlePlatform ? Math.max(formation.d, 6.3) : formation.d;
    const gap = formation.gap;
    ...
    const platform = {
      ...
      spiralStair,
      circlePlatform,
      bigSpinner: allowObstacles && (formation.bigSpinnerPlan || state.endlessCount % 26 === 0),
